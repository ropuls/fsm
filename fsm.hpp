...
public:
    ...
    auto feed(Event evt) {
        ...
        std::visit([&](auto && current_state){
            ...
            auto i = match<TransitionTable, current_state_type, Event>();
            printf("next state index: %zd\n", i);
...
*/
        ...
        */
        ...
        printf("feeding event: %s\n", type_name<Event>().c_str());
        printf("---\n\n");
        std::this_thread::sleep_for(std::chrono::seconds(1));

        std::get<next_state_t>(m_state)(evt, [this](auto && result) {
            feed(result);
        });
